			...
			const struct pci_device_id *id)
{
	...
	/*TC Hsu:	If P_+3VSO_SYNC_5 is low, the +3V down converter is auto mode(auto
	 *		switch between PWM mode and power save mode);
	 *		If P_+3VSO_SYNC_5 is high, the +3V down converter keep at PWM mode,
	 *		and will increase the power consume(more than 5mA).
	 *		So, for power consumption issue, we change the state of P_+3VSO_SYNC_5
	 *		from high to low when booting, there are many other project do the
	 *		same thing(like ME302C, FE175CG etc...), but maybe we can just change
	 * 		the default state in the GPIO table(IAFW)?
	 *
	 *PS. 		If set the +3V down converter to auto mode, device will produce noise
	 * 		when early suspend, maybe it will has side effect when record and skype
	 * 		talking if the internal micphone is too close the +3V down converter.
	 */
    ...
    if (gpio_3v > 0) {
       ...
       if (ret) {
			dev_err(&pdev->dev,"%s : failed to request P_+3VSO_SYNC_5 (gpio %d)\n", __func__, gpio_3v);
       }

       if (gpio_get_value(gpio_3v)==0) {
          printk("%s: P_+3VSO_SYNC_5 (gpio %d): LOW\n", __func__, gpio_3v);
       } else {
          printk("%s: Set P_+3VSO_SYNC_5 (gpio %d) to LOW\n", __func__, gpio_3v);
