...
static BLOCKING_NOTIFIER_HEAD(hsi_notifier_list);

/**
 * hsi_register_notify - register a notifier callback
 */
void hsi_register_notify(struct notifier_block *nb)
{
	blocking_notifier_chain_register(&hsi_notifier_list, nb);
}
EXPORT_SYMBOL_GPL(hsi_register_notify);

/**
 * hsi_unregister_notify - unregister a notifier callback
 */
void hsi_unregister_notify(struct notifier_block *nb)
{
	blocking_notifier_chain_unregister(&hsi_notifier_list, nb);
}
EXPORT_SYMBOL_GPL(hsi_unregister_notify);

void hsi_notify_tx_timeout(void)
{
	blocking_notifier_call_chain(&hsi_notifier_list, 1, NULL);
}
EXPORT_SYMBOL_GPL(hsi_notify_tx_timeout);
