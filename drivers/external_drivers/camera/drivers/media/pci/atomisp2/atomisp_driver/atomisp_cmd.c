...
static inline void atomisp_set_sensor_mipi_to_isp(
                unsigned int pixelformat,    // ASUS_BSP +++ get raw data
                struct atomisp_sub_device *asd,
                enum atomisp_input_stream_id stream_id,
                struct camera_mipi_info *mipi_info)
{
	...
	if (mipi_info->input_format != -1) {
...
#else
                {
                        ...
                        atomisp_css_input_set_format(asd, stream_id, mipi_info->input_format);
                }
                // ASUS_BSP --- get raw data
	}
	...
	if (isp->inputs[asd->input_curr].type != TEST_PATTERN &&
		...
		if (!mipi_info) {
			...
			return -EINVAL;
                ...
                // ASUS_BSP +++ get raw data
                atomisp_set_sensor_mipi_to_isp(pix->pixelformat, asd, stream_index, mipi_info);
